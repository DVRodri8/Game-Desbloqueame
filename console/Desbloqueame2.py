...
def pedir_selector():
	...
	bandera = True
	while bandera:
		selector = raw_input("Inserte el numero de la opcion deseada: ")
...
def pedir_nivel(nivel_max, dt, records):
	...
	bandera = True
	while bandera:
		nivel = raw_input("Inserte el numero de la opcion deseada: ")
...
def imprimir_tiempo(records):
	...
	raw_input('INTRO PARA VOLVER AL MENU')
...
def pedir_color():
	...
	bandera = True
	while bandera:
		noc = raw_input("Inserte el numero de la opcion deseada: ")
		if noc.isdigit():
			noc = int(noc)
			if noc >= 1 and noc <= 6:
				bandera = False
...
def jugar(nivel, niveles, records, noc):
	...
	while not park.terminado():
		jugadas = raw_input('Inserte su/s jugada/s: ')
			...
			#Si simplemente se pasan el nivel, condicion = None
				...
				while condicion == 1:
					...
					if condicion == None and nivel != niveles[0]:
						ask_for_next = raw_input("¿Desea pasar al siguiente nivel? [S/N] ")
